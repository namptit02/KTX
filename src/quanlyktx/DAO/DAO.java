...
public class DAO {
    ...
    public DAO() {
        String url = "jdbc:mysql://localhost:3306/QLY_KTX_Mk3";
        String user = "root"; // Replace with your MySQL username
        String password = "Dungthieu1972"; // Replace with your MySQL password
...
public boolean updateThoiHan(String mssv, int thoiHan) {
    if (mssv == null || mssv.trim().isEmpty()) {
        // Handle the null or empty case as per your requirements
        System.err.println("MSSV is null or empty");
        return false;
    }

    String sql = "UPDATE HopDong SET TinhTrang = ? WHERE MSSV = ?";
    try {
        PreparedStatement ps = conn.prepareStatement(sql);
        ps.setInt(1, thoiHan);
        ps.setString(2, mssv.trim());
        return ps.executeUpdate() > 0;
    } catch (Exception e) {
        e.printStackTrace();
    }
    ...
    public ArrayList<Phong> getListRoomByIDRange(String id) {
        ArrayList<Phong> list = new ArrayList<>();
        String sql = "SELECT * FROM Phong WHERE MaDay = '" + id.trim() + "'";
    ...
    public ArrayList<Phong> getTenPhong(String id) {
        ArrayList<Phong> list = new ArrayList<>();
        String sql = "SELECT MaPhong FROM Phong WHERE MaDay = '" + id.trim() + "'";
